...
export default function Deployment() {
  ...
  return (
    <Layout>
      ...
      <div className='relative rounded-lg sm:rounded-xl border border-zinc-800 bg-[#0c0c0c] overflow-hidden shadow-sm h-[50vh] sm:h-[65vh] flex flex-col'>
        <div className='flex items-center justify-between px-4 py-2 border-b border-zinc-800/80 bg-zinc-900/50'>
          <div className='flex items-center gap-2'>
            ...
            <span className='text-[11px] font-medium text-zinc-500 ml-2 font-mono uppercase tracking-wider'>
              Console Output
            </span>
          </div>
          {live && (
        ...
        <div className='flex-1 overflow-y-auto p-4 py-5 font-mono text-[13px] leading-relaxed select-text scroll-smooth'>
          {logs.map((log, i) => (
            ...
            >
              <span className='text-zinc-700 select-none w-8 text-right opacity-50 group-hover:opacity-100 transition-opacity text-[11px] pt-0.5 shrink-0'>
                {i + 1}
              </span>
              <div
                className={`whitespace-pre-wrap break-all flex-1 ${lineColor[log.stream] || 'text-zinc-300'}`}
              >
                {log.message}
              </div>
            </div>
          ...
          {!live && logs.length === 0 && (
            <div className='text-zinc-500 italic pl-12 flex items-center h-full'>
