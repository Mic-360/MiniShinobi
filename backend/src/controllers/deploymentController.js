...
const {
  ...
  getProject,
  upsertProject,
} = require('../services/runtimeRegistry');
...
function updateDeployment(id, fields) {
  ...
  db.prepare(`UPDATE deployments SET ${sets} WHERE id = ?`).run(
    ...keys.map((k) => fields[k]),
    id,
  );
}
...
function resolveProjectCommands({ project, projectPath, onLog }) {
  ...
  try {
    ...
    if (fileConfig)
      onLog('system', 'Found .minishinobi.json, applying command overrides');
  } catch (err) {
...
async function createWebhookDeployment({ repoUrl, ref }) {
  ...
  let project = allProjects.find(
    (p) => normalizeRepoUrl(p.repo_url) === normalizedIncoming,
  );
  ...
  if (!project) {
    ...
    db.prepare('UPDATE projects SET repo_url = ?, branch = ? WHERE id = ?').run(
      repoUrl,
      branch,
      project.id,
    );
  ...
  const { lastInsertRowid: deploymentId } = db
    .prepare(
...
async function executeDeployment(deploymentId, onLog) {
  ...
  const projectName = sanitizeProjectName(
    project.slug || project.name || projectNameFromRepoUrl(project.repo_url),
  );
  ...
  try {
    ...
    const resolved = resolveProjectCommands({
      project,
      projectPath,
      onLog: log,
    });
    db.prepare('UPDATE projects SET framework = ? WHERE id = ?').run(
      resolved.framework,
      project.id,
    );
